...
22. **[2026-06-17] Desktop web is full-width; only card lists get responsive constraints**
   Do instead: do not cap the root web app shell to phone width. Keep `GestureHandlerRootView`/root layout full-width, and use `apps/mobile/hooks/useResponsiveColumns.ts` only for vertical card lists that would otherwise stretch edge-to-edge. Horizontal carousels and horizontal-row cards are already fine — leave them.

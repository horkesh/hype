...
4. **[2026-05-16] Venue matching for events must handle abbreviated raw signals via reverse-substring + category disambiguation**
   Do instead: when a ticket site gives a short venue signal like "Skenderija" or "ZETRA", the canonical venue is usually a longer name ("Dom Mladih Skenderija", "Olimpijska dvorana Juan Antonio Samaranch (Zetra)"). Use `matchVenue` in `backend/src/services/venueMatch.ts` which tries: exact → forward partial → reverse partial → fuzzy variants. The reverse-partial step disambiguates by `EVENT_CATEGORIES` (concert_hall, theatre, cinema, club, cultural_center, outdoor, etc.) so "Grbavica" routes to "Stadion Grbavica" (outdoor) instead of "Pekara Grbavica" (bakery). When a raw signal hits multiple venues and none are in event categories, the matcher returns null rather than auto-picking — leave it for curation.
